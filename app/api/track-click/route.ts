...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log("TRACK CLICK BODY:", body);
    ...
    const allowedClickTypes = ["COUPON", "CASHBACK", "BANNER", "STORE"];

    if (!allowedClickTypes.includes(normalizedClickType)) {
    ...
    let resolvedUserEmail: string | null =
      ...
      typeof legacySession?.email === "string" && legacySession.email.trim()
    ...
    const redirectUrl = appendTrackingParam(targetUrl, paramKey, trackingCode);
